'use client'

import Image from 'next/image'
import { useEffect } from 'react'
import { useAuth } from '~/shared/context/auth-provider'

const Header = () => {
    const { avatar, onAuthLogin, onAuthLogout } = useAuth()

    useEffect(() => {}, [])

    return (
        <div className="h-16 flex items-center px-6 bg-[rgba(255,255,255,0.98)] shadow-[rgba(0,0,0,0.07)_0px_4px_8px_0px]">
            <div className="flex-1"></div>
            <div>{avatar && <Image className="rounded-full" src={avatar} width={48} height={48} alt="" />}</div>
            <div>
                <button onClick={onAuthLogin} className="ml-4 text-sm text-gray-600 hover:text-gray-900">
                    Login
                </button>
                <button onClick={onAuthLogout} className="ml-4 text-sm text-gray-600 hover:text-gray-900">
                    Logout
                </button>
            </div>
        </div>
    )
}

export default Header
